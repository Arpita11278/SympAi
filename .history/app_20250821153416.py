...
st.set_page_config(page_title="Virtual Disease Predictor", page_icon="🩺", layout="centered")
...
st.title("🩺 Virtual Disease Predictor (Streamlit)")
st.caption("Replace sample CSVs in `sample_datasets/` with your own larger dataset.")
...
with col1:
    max_sym = st.number_input("How many dropdowns?", min_value=3, max_value=12, value=10, step=1)
with col2:
    show_probs = st.toggle("Show top-3 predictions", value=True)

options = [""] + vocab
user_syms = []
for i in range(int(max_sym)):
    user_syms.append(st.selectbox(f"Symptom {i+1}", options, key=f"s{i}"))

if st.button("Predict"):
    selected = [s for s in user_syms if s]
    if not selected:
        st.warning("Please select at least 1 symptom."); st.stop()
    model, le = train_or_load(df, vocab)
    x_vec = vectorize(selected, vocab)
    ...
    if prec_list:
        for p in prec_list: st.markdown(f"- {p}")
    else:
        st.markdown("- Drink more water\n- Rest well\n- Consult a doctor if symptoms persist")

    if show_probs and len(results) > 1:
        st.write("---"); st.write("**Top-3 predictions:**")
        for d, p in results: st.write(f"- {d}: {p:.2%}")
